...
**
**************************************************************************************/
BOOL CSerialPort1::InitPort(CWnd*  pPortOwner,	  // the owner (CWnd) of the port (receives message)
...
//
UINT CSerialPort1::CommThreadProc(LPVOID pParam)
{
	CSerialPort1 *port = (CSerialPort1*)pParam;
		...
		{ 
			...
			if(TRUE==GetCommMask(port->m_hComm, &CommEvent))
			{
				...
				{  //清除发送
					...
					{
						::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_CTS_DETECTED1, \
				...
				else if (Event & CommEvent & EV_RXFLAG)
				{
					...
					{
						::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_RXFLAG_DETECTED1,\
								     (WPARAM) 0, (LPARAM) port->m_nPortNr);
					}
				}
				...
				{	//输入打断
					...
					{
						::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_BREAK_DETECTED1, \
							         (WPARAM) 0, (LPARAM) port->m_nPortNr);
					}
				}
				...
				{	
					...
					{
						::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_ERR_DETECTED1,\
								     (WPARAM) 0, (LPARAM) port->m_nPortNr);
					}
				}
				...
				{	
					...
					{
						::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_RING_DETECTED1,\
								     (WPARAM) 0, (LPARAM) port->m_nPortNr);
					}
				}
				else
...
** func:创建线程开始监视串口
*******************************************************************/
BOOL CSerialPort1::StartMonitoring()
{
...
*******************************************************************/
BOOL CSerialPort1::RestartMonitoring()
{
...
** func:挂起线程
*******************************************************************/
BOOL CSerialPort1::StopMonitoring()
{
...
//
void CSerialPort1::ProcessErrorMessage(char* ErrorText)
{
...
BOOL CSerialPort1::userBuildCommDCB(DCB* dcb,UINT baud,UINT parity,UINT stopbit,UINT databit)
{
	EnterCriticalSection(&m_csCommunicationSync);
...
void  CSerialPort1::ReceiveChar(CSerialPort1* port)
{
...
/*		CString str;
        ...
        if(TRUE==port->m_evRxchar)
		{
			for(UINT i=0;i<dwReadLen;i++)
			{
				::SendMessage(port->m_pOwner->m_hWnd, WM_COMM_RXCHAR1, (WPARAM) recvBuf[i], (LPARAM)port->m_nPortNr);
			}
		}
	}
...
//清除读缓冲区
BOOL CSerialPort1::ClearReadBuffer()
{
...
BOOL CSerialPort1::WriteToPort(char* string,DWORD length)
{	
...
DCB CSerialPort1::GetDCB()// Return the device control block
{
	return m_dcb;
}

DWORD CSerialPort1::GetMonitorEvents()// Return the communication event masks
{
	return m_dwCommEvents;
}

DWORD CSerialPort1::GetWriteBufferSize()// Return the output buffer size
{
	return m_nWriteBufferSize;
}

DWORD CSerialPort1::GetCommPort()
{
	return  m_nPortNr;
}

void CSerialPort1::Close()
{   
	/*结束线程*/
	if(m_bThreadAlive==TRUE)
	{
...
UINT CSerialPort1::parityAnalyze(const char* ch)
{
...
UINT CSerialPort1::stopbitAnalyze(const char* ch)
{
	if(0==strncmp("1",ch,5))
	{
...
BOOL CSerialPort1::SetBaudRate(DWORD rate)  /*设置波特率*/
{
	//CString str;
...
BOOL CSerialPort1::SetParity(const char* ch)      /*设置奇偶校验*/
{
	EnterCriticalSection(&m_csCommunicationSync);
	if (TRUE==GetCommState(m_hComm, &m_dcb)){		
...
BOOL CSerialPort1::SetStopbit(const char* ch)     /*设置停止位*/
{
	EnterCriticalSection(&m_csCommunicationSync);
	if (TRUE==GetCommState(m_hComm, &m_dcb)){		
...
/*设置数据位*/
BOOL CSerialPort1::SetDatabit(BYTE bits)   
{
	EnterCriticalSection(&m_csCommunicationSync);
	if (TRUE==GetCommState(m_hComm, &m_dcb)){		
		m_dcb.ByteSize=bits;
...
/*设置缓冲区的大小*/
BOOL CSerialPort1::SetWriteBufSize(DWORD len)   
{
	EnterCriticalSection(&m_csCommunicationSync);
	if(m_szWriteBuffer!=NULL)
	{
...
BOOL CSerialPort1::SetReadBufSize(DWORD length)
{
	EnterCriticalSection(&m_csCommunicationSync);
	if(m_szReadBuffer!=NULL)
	{
...
/*设置串口监视事件*/  
BOOL CSerialPort1::SetMonitorEvent(DWORD tmpEvent)  
{
	EnterCriticalSection(&m_csCommunicationSync);
	if(NULL!=m_hComm){
...
/*获得串口操作句柄*/
HANDLE CSerialPort1::GetCommHandle()   
{
	return m_hComm;
}

CWnd* CSerialPort1::GetWindowOwner()   /*获得串口的窗口拥有者*/
{
	return m_pOwner;
}
...
***************************************************************/
void CSerialPort1::EnableSerialMessage(BOOL EnRxchar,BOOL EnCts,BOOL EnRxFlag,\
...
/*获得用户自定义的接收数据缓冲区的大小*/
DWORD  CSerialPort1::GetReadBufferSize()
{
	return m_nReadBufferSize;
}

/*获得接收缓冲区的数据*/
BOOL  CSerialPort1::GetReadBufferData(BYTE *ch,DWORD length)
{
